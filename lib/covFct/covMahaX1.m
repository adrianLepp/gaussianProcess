function [K,dK,D2] = covMahaX1(mode, par, k, dk, hyp, x, z)
...
%this is what I did...
B1 = [ 1e-08 , 0 ; 0 , 1000]; %2 x2 matrix
h = {@h1, @h2};
for i = 1 : n
   for j = 1 : n
       s = 0;
       for k = 1 : length(B1)
           for l = 1 : length(B1)   %k = 2, l = 1 : B(2,1) * h2
               s = s + B1(k,l) * h{k}(x(i,:)) * h{k}(x(j,:));       %!
           end
       end
       K(i,j) = K(i,j) + s; 
   end
...
function h = h1(x)
    ...
    h = 1 / A * x(1,4); %1
    %h = 1 / A * sqrt(2*g*abs(x(1,3)-x(1,2)));%2
    %h = 1 / A *sqrt(2*g*abs(x(1,1)-x(1,3))); %3
end
...
function h = h2(x)
    ...
    h = - 1 / A * sqrt(2*g*abs(x(1,1)-x(1,3))); %1
    %h = -  1 / A * sqrt(2*g*abs(x(1,2))); %2 (VZ!?)
    %h = - 1 / A *sqrt(2*g*abs(x(1,3)-x(1,2))); %3
end
